...
class VideoProcessor:
    ...
    def inference_loop(self):
        global state
        while self.running:
            if getattr(self, 'latest_frame_for_yolo', None) is not None:
                frame = self.latest_frame_for_yolo
                self.latest_frame_for_yolo = None  # consume
                
                if getattr(state, 'model_loaded', False) and getattr(state, 'model', None):
    ...
    def _open_cap(source):
        ...
        if isinstance(source, str) and source.startswith('rtsp://'):
            # Force TCP transport to avoid UDP stutter / packet loss
    ...
    def update(self):
        ...
        while self.running:
            ...
            if getattr(state, 'new_source_path', None) is not None:
                if cap:
                    cap.release()
                cap = self._open_cap(state.new_source_path)
            ...
            if state.model_loaded and state.model:
                ...
                # === BATCH COMPLETE: Freeze state, only watch for auto-reset ===
                if state.outer_count == 3:
...
def get_frame():
    last_count = -1
    while True:
        if processor.update_count != last_count and processor.jpeg_bytes:
            last_count = processor.update_count
            yield (b'--frame\r\n'
                   b'Content-Type: image/jpeg\r\n\r\n' + processor.jpeg_bytes + b'\r\n')
        time.sleep(0.01)
